...
type TaskStatusOpFunc func(t *testing.T, dbo orm.DB, in *db.TaskStatus) Cleaner

func TaskStatus(t *testing.T, dbo orm.DB, in *db.TaskStatus, ops ...TaskStatusOpFunc) (*db.TaskStatus, Cleaner) {
	repo := db.NewCommonRepo(dbo)
	var cleaners []Cleaner
	...
	// Create the main entity
	taskStatus, err := repo.AddTaskStatus(t.Context(), in)
	if err != nil {
		t.Fatal(err)
	}

	return taskStatus, func() {
		// Clean up related entities from the last to the first
		for i := len(cleaners) - 1; i >= 0; i-- {
			cleaners[i]()
		}
...
func WithFakeTaskStatus(t *testing.T, dbo orm.DB, in *db.TaskStatus) Cleaner {
	if in.StatusID == 0 {
		in.StatusID = 1
	}
	...
	if in.Alias == "" {
		in.Alias = strings.ReplaceAll(cutS(gofakeit.Sentence(6), 64), " ", "-")
	}
